...
fn student_record_system() {
    let mut studentRecords = HashMap::new();
    studentRecords.insert(String::from("raj"), 20);
    ...
    loop {
        ...
        match input.trim() {
            "add" => {
                ...
                studentRecords.insert(name.trim().to_string(), num_age);
            }
            "get" => {
                println!("Enter student name");
                let mut name = String::new();
                io::stdin().read_line(&mut name).expect("failed");
                match studentRecords.get(name.trim()) {
                    Some(age) => println!("Age: {age}"),
                    None => println!("Student not found"),
                }
            }
            "list" => {
                for (name, age) in &studentRecords {
                    println!("{} {}", name, age);
                }
            }
            "quit" => break,
